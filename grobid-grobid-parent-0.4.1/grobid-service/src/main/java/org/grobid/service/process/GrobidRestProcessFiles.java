...
import org.grobid.core.document.CreateTrainingHeader;
...
public class GrobidRestProcessFiles {
    ...
    public static Response processCreateTrainingHeader(final InputStream inputStream) {

        LOGGER.debug(methodLogIn());
        Response response = null;
        CreateTrainingHeader retVal = null;
        boolean isparallelExec = GrobidServiceProperties.isParallelExec();
        File originFile = null;
        Engine engine = null;
        try {
            ...
            if (originFile == null) {
                ...
                engine = Engine.getEngine(isparallelExec);
                if (isparallelExec) {
                    retVal = engine.createTrainingHeader(originFile.getAbsolutePath());
                    //retVal = engine.segmentAndProcessHeader(originFile.getAbsolutePath(), consolidate, null);
                    GrobidPoolingFactory.returnEngine(engine);
                    engine = null;
                } else {
