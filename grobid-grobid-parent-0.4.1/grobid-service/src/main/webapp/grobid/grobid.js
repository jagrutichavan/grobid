...
var grobid = (function($) {
	...
	function SubmitSuccesful(responseText, statusText, xhr) {
		...
		var selectedService1 = $('#selectedService option:selected').attr('value');
		if(selectedService1 == "createTrainingHeader") {
		    var v = JSON.parse(responseText)
		    var tieButtons = '<button id="headerTei" class="btn" style="margin-left:100px">Header Tei</button><button id="authorTei" class="btn" style="margin-left:100px">Author Tei</button><button id="affiliationTei" class="btn" style="margin-left:100px">Affiliation Tei</button><br>';

		    var headerTieResult = tieButtons + '<div id="headerTeiResult"><pre class="prettyprint lang-xml" id="xmlCode">';
		    if(v.header == null) {
		        v.header = "No Header Content";
		    }
		    var testStr1 = vkbeautify.xml(v.header);
		    headerTieResult += htmll(testStr1);
            headerTieResult += "</pre></div>";

            var authorTieResult = '<div id="authorTeiResult" style="display:none"><pre class="prettyprint lang-xml" id="xmlCode">';
            if(v.author == null) {
                v.author = "No Author Content";
            }
            var testStr1 = vkbeautify.xml(v.author);
            authorTieResult += htmll(testStr1);
            authorTieResult += "</pre></div>";

            var affTieResult = '<div id="affTeiResult" style="display:none"><pre class="prettyprint lang-xml" id="xmlCode">';
            if(v.affiliation == null) {
                v.affiliation = "No Affiliation Content";
            }
            var testStr1 = vkbeautify.xml(v.affiliation);
            affTieResult += htmll(testStr1);
            affTieResult += "</pre></div>";
            var finalHtml = headerTieResult+authorTieResult+affTieResult;

            $('#requestResult').html('');
		    $("#createTraingingResultDiv").html(finalHtml);
		    window.prettyPrint && prettyPrint();
            $('#createTraingingResultDiv').show();
            $("#authorTeiResult").hide();
            $("#affTeiResult").hide();
            $("#headerTeiResult").show();
            $("#headerTei").click(function() {
                $("#authorTeiResult").hide();
                $("#affTeiResult").hide();
                $("#headerTeiResult").show();
                return false;
            ...
            $("#authorTei").click(function() {
                $("#authorTeiResult").show();
                $("#affTeiResult").hide();
                $("#headerTeiResult").hide();
                return false;
            });

            $("#affiliationTei").click(function() {
                $("#authorTeiResult").hide();
                $("#affTeiResult").show();
                $("#headerTeiResult").hide();
                return false;
...
//console.log(page);
	...
	function processChange() {
		...
		} else if (selected == 'createTrainingHeader') {
          			createInputFile(selected);
          			//$('#consolidateBlock2').show();
          			setBaseUrl('createTrainingHeader');
          		}
	}
	...
	function createInputFile4(selected) {
    		...
    		$('#textInputDiv').hide();
    		$('#fileInputDiv').show();
    		var innerTag = '<button type="button" class="btn" style=" margin-left: 100px; ">Author TEI</button> <button type="button" class="btn" style=" margin-left: 100px; ">Author TEI</button><button type="button" class="btn" style=" margin-left: 100px; ">Author TEI</button>';
    		$('#createTraingingResultDiv').append('');
