...
public class EmailSanitizer {
    ...
    private static final Pattern[] EMAIL_STRIP_PATTERNS = new Pattern[] {
            Pattern.compile("^(e\\-mail|email|e\\smail|mail):",Pattern.CASE_INSENSITIVE),
    ...
    private static final Pattern EMAIL_SPLITTER_PATTERN = Pattern.compile("(\\sor\\s|,|;|/)",Pattern.CASE_INSENSITIVE);
...
//
            ...
            if (splitEmails.size() > 1) {
                ...
                if (atSeparatedStrings.size() == 2) {
                    ...
                    //amn-mod prevent eg alpaslantanoglu@yahoo.com,Phone:+90 216 5422020
                    if (atIndex!=-1) {
            ...
            //amn-mod handles extracting only valid email contents
            //***********
            else if (splitEmails.size()>0){
                    String regex="(?:[A-Z.a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\\x01-\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])";
                    Pattern p=Pattern.compile(regex);
                    Matcher m=p.matcher(splitEmails.remove(0));
                    while (m.find()){
                        splitEmails.add(m.group());
                    }
            }
            //***********
            for (String splitEmail : splitEmails) {
                ...
                if (email != null && !email.isEmpty()) {
                    ...
                    //amn-mod
                    //final santity check on invidual email
                    String regex="(?:[A-Z.a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|\"(?:[\\x01-\\x0b\\x0c\\x0e-\\x1f\\x21\\x23-\\x5b\\x5d-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\\x01-\\x0b\\x0c\\x0e-\\x1f\\x21-\\x5a\\x53-\\x7f]|\\\\[\\x01-\\x09\\x0b\\x0c\\x0e-\\x7f])+)\\])";
                    Pattern p=Pattern.compile(regex);
