...
//        tei.append(biblio.toTEIAuthorBlockNew(6));

        if(biblio.getArticleNote() != null){
            tei.append("\t\t\t\t\t\t<articlenote>\n");
            tei.append(TextUtilities.HTMLEncode(biblio.getArticleNote())
                    + "</articlenote>");
        }

        //miscellaneous unmapped contact
        if (biblio.getUnmappedPhones() != null || biblio.getUnmappedUrls() != null || biblio.getUnmappedEmails() != null) {
            tei.append("\t\t\t\t\t\t<miscellaneous>\n");
            if (biblio.getUnmappedPhones() != null) {
                for (String ph : biblio.getUnmappedPhones()) {
                    tei.append("\t\t\t\t\t\t\t");
                    tei.append("<phone>" + TextUtilities.HTMLEncode(ph.trim()) + "</phone>\n");
                }
            }
            if (biblio.getUnmappedUrls() != null) {
                for (String wb : biblio.getUnmappedUrls()) {
                    tei.append("\t\t\t\t\t\t\t");
                    tei.append("<url>" + TextUtilities.HTMLEncode(wb.trim()) + "</url>\n");
                }
            }
            if (biblio.getUnmappedEmails() != null) {
                for (String eml : biblio.getUnmappedEmails()) {
                    tei.append("\t\t\t\t\t\t\t");
                    tei.append("<email>" + TextUtilities.HTMLEncode(eml.trim()) + "</email>\n");
                }
            }
            tei.append("\t\t\t\t\t\t</miscellaneous>\n");
        }
