...
import org.grobid.core.data.util.*;
...
public class BiblioItem {
    ...
    private List<ITag> sequenceTags = null;
    private boolean doesAnyAuthorHasAstrik = false;
    private boolean doesHasContextualWord = false;
    private List<Marker> symbolsMarkers = new ArrayList<>();
    ...
    private List<String> unmappedPhones;
    private List<String> unmappedUrls;
    private List<String> unmappedEmails;
    private String corresEmails = "";
    private String corresAuthors = "";
...
//    private List<Person> fullAuthors = null;
    ...
    private List<Boolean> corresFlagList = new ArrayList<Boolean>();

    private String articleNote = null;
    ...
    public void setSequenceTag(List<ITag> tags) {
        sequenceTags = tags;
    }

    public List<ITag> getSequenceTags() {
        return sequenceTags;
    }
    ...
    public List<String> getUnmappedPhones(){return unmappedPhones; }

    public List<String> getUnmappedUrls(){return unmappedUrls; }

    public List<String> getUnmappedEmails(){return unmappedEmails; }
    ...
    public void setArticleNote(String ti) {
        articleNote = ti;
    }
    ...
    public void addFullAuthor(Person aut) {
        ...
        aut.modifyAuthoronEnvelop();
        if (!fullAuthors.contains(aut)) {
            fullAuthors.add(aut);
        } else {
            int idx = fullAuthors.indexOf(aut);
            Person author = fullAuthors.get(idx);
            author.update(aut);
            fullAuthors.remove(idx);
            fullAuthors.add(idx, author);
        }
    }
    ...
    public void addUnmappedPhones(String ph){
        if (unmappedPhones == null){
            unmappedPhones = new ArrayList<String>();
        }
        if (!unmappedPhones.contains(ph)){
            unmappedPhones.add(ph);
        }
    }

    public void addUnmappedUrls(String wb){
        if (unmappedUrls == null){
            unmappedUrls = new ArrayList<String>();
        }
        if (!unmappedUrls.contains(wb)){
            unmappedUrls.add(wb);
        }
    }

    public void addUnmappedEmails(String wb){
        if (unmappedEmails == null){
            unmappedEmails = new ArrayList<String>();
        }
        if (!unmappedEmails.contains(wb)){
            unmappedEmails.add(wb);
        }
    }


    public void addCorresFlagtoList(boolean corresFlag) {
        corresFlagList.add(corresFlag);
    }

    public List<Boolean> getCorresFlagsList() {
        return corresFlagList;
    }
    ...
    public void setCorresEmails(String e){
        corresEmails += e;
    }

    public void setCorresAuthors(String e){
        corresAuthors = e;
    }

    public String getCorresAuthors(){
        return corresAuthors;
    }

    public void addUniqueAuthor(Person aut) {
        if (fullAuthors == null)
            fullAuthors = new ArrayList<Person>();
        aut.modifyAuthoronEnvelop();
        if (!isAuthorNameDuplicate(aut))
            fullAuthors.add(aut);
    }

    private boolean isAuthorNameDuplicate(Person newAuthor) {
        for (Person uniqueAuthor : fullAuthors) {
            if (isDuplicateAuthor(newAuthor, uniqueAuthor)) {
                setAuthorDegreeAndPrefix(newAuthor, uniqueAuthor);
                return true;
            }
        }
        return false;
    ...
    private void setAuthorDegreeAndPrefix(Person newAuthor, Person uniqueAuthor)
    {
        if(newAuthor.getPrefix() != null)
            uniqueAuthor.setPrefix(newAuthor.getPrefix());

        if(newAuthor.getAuthorDegree() != null)
            uniqueAuthor.setAuthorDegree(newAuthor.getAuthorDegree());
    }

    public boolean isDuplicateAuthor(Person newAuthor, Person uniqueAuthor) {
        if (newAuthor.getFirstName() == null && newAuthor.getMiddleName() == null && newAuthor.getLastName() == null)
            return false;

        String uniqueAuthorFullName = getFullName(uniqueAuthor);
        String newAuthorFullName = getFullName(newAuthor);

        return uniqueAuthorFullName.toLowerCase().equals(newAuthorFullName.toLowerCase());
    }

    private String getFullName(Person author) {
        return getName(author.getFirstName()) + getName(author.getMiddleName()) + getName(author.getLastName()) +
                getName(author.getParticle()) + getName(author.getSuffix());
    }
    ...
    private String getName(String name) {
        ...
        return  name.replaceAll(String.format(RegexPattern.CHARACTER_GROUP, RegexPattern.SPACE + RegexPattern.PERIOD +
                RegexPattern.COLON + RegexPattern.COMMA + RegexPattern.SEMICOLON + RegexPattern.N_DASH + RegexPattern.EM_DASH +
                RegexPattern.SINGLE_STRAIGHT_QUOTE + RegexPattern.SINGLE_OPENING_QUOTE + RegexPattern.SINGLE_CLOSING_QUOTE +
                RegexPattern.HYPHEN_MINUS), "");
    }

    public boolean isAffiliationUnique(Affiliation newAffiliation, List<Affiliation> uniqueAffiliations) {
        newAffiliation.setText(getRegexEscapeString(newAffiliation.getText()));

        if (uniqueAffiliations.size() == 0)
            return true;

        for (Affiliation uniqueAffiliation : uniqueAffiliations) {
            Boolean isOrganizationUnique = isOrganizationUnique(newAffiliation, uniqueAffiliation);
            if (!isOrganizationUnique) {
                uniqueAffiliation.setText(uniqueAffiliation.getText() + RegexPattern.OR + newAffiliation.getText());
                setAddress(newAffiliation, uniqueAffiliation);
                return false;
            }
        }
        return true;
    ...
    private String getRegexEscapeString(String text)
    {
        return "\\Q" + text + "\\E";
    }

    private Boolean isOrganizationUnique(Affiliation newAffiliation, Affiliation uniqueAffiliation) {
        boolean isDepartmentUnique = false;

        if (isAddressUnique(newAffiliation, uniqueAffiliation)) return true;

        boolean isInstituteUnique = isInstituteOrDepartmentUnique(newAffiliation.getInstitutions(), uniqueAffiliation.getInstitutions(),
                newAffiliation.getDepartments());

        if (!isInstituteUnique)
            isDepartmentUnique = isInstituteOrDepartmentUnique(newAffiliation.getDepartments(), uniqueAffiliation.getDepartments(),
                    newAffiliation.getInstitutions());

        return isInstituteUnique || isDepartmentUnique;
    }

    public boolean isAddressUnique(Affiliation newAffiliation, Affiliation uniqueAffiliation){
        if(isCountryUnique(newAffiliation, uniqueAffiliation)) return true;
        if(isAddressElementDifferent(newAffiliation.getSettlement(), uniqueAffiliation.getSettlement())) return true;
        if(isAddressElementDifferent(newAffiliation.getRegion(), uniqueAffiliation.getRegion())) return true;
        if(isAddressElementDifferent(newAffiliation.getPostCode(), uniqueAffiliation.getPostCode())) return true;
        if(isAddressElementDifferent(newAffiliation.getPostBox(), uniqueAffiliation.getPostBox()))  return true;

        return false;
    }

    private boolean isAddressElementDifferent(String newAffiliationAddress, String uniqueAffiliationAddress){
        if(newAffiliationAddress != null && uniqueAffiliationAddress !=null)
            return !newAffiliationAddress.toLowerCase().equals(uniqueAffiliationAddress.toLowerCase());

            return false;
    }

    public boolean isCountryUnique(Affiliation newAffiliation, Affiliation uniqueAffiliation) {
        if (uniqueAffiliation.getCountry() != null && newAffiliation.getCountry() != null) {
            String uniqueAffiliationCountryName = getPunctuationRemovedCountryName(uniqueAffiliation.getCountry());
            String newAffiliationCountryName = getPunctuationRemovedCountryName(newAffiliation.getCountry());

            if(uniqueAffiliationCountryName.toLowerCase().equals(newAffiliationCountryName.toLowerCase())) return false;

            String uniqueAffiliationCountryKey = getUniqueAffiliationCountryKey(uniqueAffiliation, uniqueAffiliationCountryName);

            if (uniqueAffiliationCountryKey == null)
                return true;

            String newAffiliationCountryKey = getCountryKey(newAffiliationCountryName);

            if(newAffiliationCountryKey == null)
                return true;

            return !uniqueAffiliationCountryKey.equals(newAffiliationCountryKey);
        }
        return false;
    }

    private String getPunctuationRemovedCountryName(String countryName) {
        return countryName.replaceAll(String.format(RegexPattern.CHARACTER_GROUP, RegexPattern.PERIOD +
                RegexPattern.COMMA + RegexPattern.SINGLE_OPENING_QUOTE + RegexPattern.SINGLE_STRAIGHT_QUOTE +
                RegexPattern.HYPHEN_MINUS), "");
    }

    private String getUniqueAffiliationCountryKey(Affiliation uniqueAffiliation, String uniqueAffiliationCountryName) {
        String uniqueAffiliationCountryKey;
        if (uniqueAffiliation.getCountryKey() == null) {
            uniqueAffiliationCountryKey = getCountryKey(uniqueAffiliationCountryName);
            if(uniqueAffiliationCountryKey != null)
                uniqueAffiliation.setCountryKey(uniqueAffiliationCountryKey);
        }
        else
            uniqueAffiliationCountryKey = uniqueAffiliation.getCountryKey();

        return uniqueAffiliationCountryKey;
    }

    private String getCountryKey(String CountryName) {
        Lexicon lexicon = Lexicon.getInstance();
        return lexicon.getcountryCode(CountryName);
    }

    private boolean isInstituteOrDepartmentUnique(List<String> newOrganizations, List<String> uniqueOrganizations, List<String> notNullOrganization) {
        return (newOrganizations == null && notNullOrganization != null && uniqueOrganizations != null) ||
                (newOrganizations != null && uniqueOrganizations == null) ||
                isOrganizationUniqueWhenValueIsNotNull(newOrganizations, uniqueOrganizations);
    }

    private boolean isOrganizationUniqueWhenValueIsNotNull(List<String> newOrganizations, List<String> uniqueOrganizations) {
        if (uniqueOrganizations != null && newOrganizations != null) {
            if ((uniqueOrganizations.size() == newOrganizations.size())) {
                for (String newOrganization : newOrganizations) {
                    boolean isOrganizationMatched = false;
                    for (int index = 0; index < uniqueOrganizations.size(); index++) {
                        String uniqueOrganization = uniqueOrganizations.get(index);
                        if (uniqueOrganization.toLowerCase().equals(newOrganization.toLowerCase())) {
                            isOrganizationMatched = true;
                            break;
                        }
                    }
                    if(!isOrganizationMatched) return true;
                }
            }
            if ((uniqueOrganizations.size() != newOrganizations.size())) return true;
        }
        return false;
    }

    private void setAddress(Affiliation newAffiliation, Affiliation uniqueAffiliation)
    {
        if(doesAddressNeedToUpdate(newAffiliation.getAddrLine(), uniqueAffiliation.getAddrLine()))
            uniqueAffiliation.setAddrLine(newAffiliation.getAddrLine());

        if(doesAddressNeedToUpdate(newAffiliation.getPostBox(), uniqueAffiliation.getPostBox()))
            uniqueAffiliation.setPostBox(newAffiliation.getPostBox());

        if(doesAddressNeedToUpdate(newAffiliation.getPostCode(), uniqueAffiliation.getPostCode()))
            uniqueAffiliation.setPostCode(newAffiliation.getPostCode());

        if(doesAddressNeedToUpdate(newAffiliation.getSettlement(), uniqueAffiliation.getSettlement()))
            uniqueAffiliation.setSettlement(newAffiliation.getSettlement());

        if(doesAddressNeedToUpdate(newAffiliation.getRegion(), uniqueAffiliation.getRegion()))
            uniqueAffiliation.setRegion(newAffiliation.getRegion());

        if(doesAddressNeedToUpdate(newAffiliation.getCountry(), uniqueAffiliation.getCountry()))
            uniqueAffiliation.setCountry(newAffiliation.getCountry());
    }

    private boolean doesAddressNeedToUpdate(String newAddress, String uniqueAddress) {
        return (uniqueAddress == null && newAddress != null)
                || (uniqueAddress != null && newAddress != null &&
                uniqueAddress.length() < newAddress.length());
    }
    ...
    public void attachEmails() {
	    ...
	    // authorEmailAssigner.assign(fullAuthors, emailles);
            ...
            setCorresAttrToAuthor(fullAuthors,corresEmails);
            setAuthorSymbolMarker();

            Boolean doesAuthorHasEmail = false;
            for(String email : sanitizedEmails){
                for(Person per : fullAuthors){
                    if(per.getEmail() != null && per.getEmail().contains(email)){
                        doesAuthorHasEmail =  true;
                        break;
                    }
                }

                if(!doesAuthorHasEmail){
                    if(!doesSymbolMarkerEmailMapWithAuthor(email)){
                        addUnmappedEmails(email);
                    }
                }
                doesAuthorHasEmail = false;
            }
	    //*********
        }
    }

    private boolean doesSymbolMarkerEmailMapWithAuthor(String email) {
        if(symbolsMarkers != null && symbolsMarkers.size() > 0){
            return canSetNearestSymbolMarker(email);
        }
        return false;
    }

    // this logic work if marker preceded by email
    private boolean canSetNearestSymbolMarker(String email) {
        String [] splitText= this.email.split(email);
        if(splitText.length > 0) {
            List<Integer> probableSymbolMarkerIndex = getEmailSymbolMarker(splitText[0]);
            if (checkAllProbableSyblMarkerIsSame(probableSymbolMarkerIndex)) {
                return assignEmailToAuthor(splitText[0], email);
            }
        }
        return false;
    }

    private List<Integer> getEmailSymbolMarker(String s) {
        List<Integer> probableSymbolMarkerIndex = new ArrayList<>();
        int index = 0;

        for(Marker symblMar:this.symbolsMarkers){
            if(s.contains(symblMar.getText())){
                probableSymbolMarkerIndex.add(index);
            }
            index += 1;
        }
        return probableSymbolMarkerIndex;
    }

    private boolean checkAllProbableSyblMarkerIsSame(List<Integer> probableSymbolMarkerIndex) {
        String oldSybMarker = null;
        for (int index:probableSymbolMarkerIndex){
            if(oldSybMarker != null) {
                String newSybMarker = this.symbolsMarkers.get(index).getType();
                if(!oldSybMarker.equals(newSybMarker)){
                    return false;
                }
            }
            oldSybMarker = this.symbolsMarkers.get(index).getType();
        }
        return true;
    }

    private boolean assignEmailToAuthor(String string, String email) {
        Person per = getPerson(string);
        if(per!= null){
            per.setEmail(email);
            return true;
        }
    ...
    private Person getPerson(String string) {
        String nearestSymbol = getCorrectNearestSymbol(string);
        for(Marker mark: this.symbolsMarkers){
            if(mark.getText().equals(nearestSymbol)){
                for (Person per: fullAuthors){
                    if(mark.getPerson().equals(per)){
                        return per;
                    }
                }
            }
    ...
    private String getCorrectNearestSymbol(String string) {
        String symbol = this.symbolsMarkers.get(0).getType();
        List<Integer> lst = getAllSubStringOccurenceIndex(string, symbol);
        ...
        while (index > -1) {
            if(oldChar != null){
                newChar = string.charAt(lst.get(index));
                if(oldChar.equals(newChar) && ((lstIndex -1) == lst.get(index))) {
                    nearestMarker += newChar.toString();
                } else {
                    break;
                }
                oldChar = string.charAt(lst.get(index));
            }else {
                oldChar = string.charAt(lst.get(index));
                nearestMarker = oldChar.toString();
            }
            lstIndex = lst.get(index);
            index -= 1;
        }
        return nearestMarker;
    }

    private List<Integer> getAllSubStringOccurenceIndex(String string, String findingTex){
        List<Integer> lst = new ArrayList<>();
        int index = string.indexOf(findingTex);
        addToList(lst, index);
        while(index >= 0) {
            index = string.indexOf(findingTex, index+1);
            addToList(lst, index);
        }
        return lst;
    }

    private void addToList(List<Integer> lst, int index) {
        if(index > -1){
            lst.add(index);
        }
    }

    private void setAuthorSymbolMarker() {
        addAuthorSymbolMarker();
        if(this.symbolsMarkers.size() > 0){
            removeRepeatedMarker(
                    getRepeatedMarkers(
                            getRepeatedSymbolMarker()
                    )
            );
        }
    }

    private List<Marker> getRepeatedMarkers(List<String> repeatedSymbolMarkers) {
        List<Marker> repeatedMarker = new ArrayList<>();

        for(String repeatedSymbol: repeatedSymbolMarkers){
            for(int index=0; index < this.symbolsMarkers.size(); index++){
                Marker marker =  this.symbolsMarkers.get(index);
                if(repeatedSymbol.equals(marker.getText())){
                    repeatedMarker.add(marker);
                }
            }
        }
        return repeatedMarker;
    }

    private void removeRepeatedMarker( List<Marker> repeatedMarker) {
        if(repeatedMarker.size() > 0) {
            for (Marker marker : repeatedMarker) {
                this.symbolsMarkers.remove(marker);
            }
        }
    }

    private List<String> getRepeatedSymbolMarker() {
        List<String> repeatedSymbolMarkers = new ArrayList<>();
        for (int firstIndex=0; firstIndex < this.symbolsMarkers.size(); firstIndex++){
            Marker firstMarker = getSymbolMarkerByIndex(firstIndex);
            for (int secondIndex= firstIndex + 1; secondIndex < this.symbolsMarkers.size(); secondIndex++){
                Marker secondMarker = getSymbolMarkerByIndex(secondIndex);
                if(firstMarker.getText().equals(secondMarker.getText())){
                    if(!repeatedSymbolMarkers.contains(firstMarker.getText())) {
                        repeatedSymbolMarkers.add(firstMarker.getText());
                    }
                }
            }
        }
        return repeatedSymbolMarkers;
    }

    private Marker getSymbolMarkerByIndex(int ind) {
        return this.symbolsMarkers.get(ind);
    }

    private void addAuthorSymbolMarker() {
        for(Person per :  fullAuthors){
            if(per.getMarkers() != null && per.getMarkers().size() > 0){
                String symbMarker = "";
                boolean doessymbMarkerCont = false;
                Marker mar = null;
                for(String marker: per.getMarkers()){
                    if(marker.equals(symbMarker) && doessymbMarkerCont){
                        mar.setText(mar.getText()+marker);
                        doessymbMarkerCont = true;
                    }else if(doesHaveSymbolMarker(marker)){
                        addSymbolMarker(mar);
                        mar = new Marker();
                        mar.setType(marker);
                        mar.setText(marker);
    ...
    private void addSymbolMarker(Marker mar) {
        if(mar != null){
            symbolsMarkers.add(mar);
        }
    }

    private boolean doesHaveSymbolMarker(String marker) {
        return Utility.doesStringMatchWithPattern(marker, RegexPattern.NOT_ALPHA_NUMERIC);
    }

    /**
     * Attach existing reconized url to authors
     */
    public void attachUrls(String res){
        if (web == null){
            return;
        }
        ...
        while(m.find()){
            if(m.group(0)!=null){
                wb = m.group(0).trim();
                String em = AuthorUrlAssigner.mapUrlEmail(res,wb);
                if (em != null && !em.equals("notFound")) {
                    String urlEmail = wb + "\t" + em;
                    AuthorUrlAssigner.assign(fullAuthors, urlEmail);
                } else {
                    //save unmapped url to put under miscellaneous tag
                    addUnmappedUrls(wb);
                }
    ...
    /**
     * Attach existing recognized phones to authors
     */
    public void attachPhones(String res) {
        if (phoneList == null || phoneList.size() == 0) {
            return;
        }
        for (String ph: phoneList) {
                String em = AuthorPhoneAssigner.mapPhoneEmail(res, ph);
                if (em != null && !em.equals("notFound")) {
                    String phoneEmail = ph + "\t" + em;
                    AuthorPhoneAssigner.assign(fullAuthors, phoneEmail);
                } else {
                    //save unmapped phone to put under miscellaneous tag
                    addUnmappedPhones(ph);
                }

                Boolean doesAuthorHasPhone = false;
                for (Person per : fullAuthors) {
                    if ((per.getPhones() != null && per.getPhones().contains(ph)) || (unmappedPhones != null && unmappedPhones.contains(ph))) {
                        doesAuthorHasPhone = true;
                        break;
                    }
                }

                if (!doesAuthorHasPhone) {
                    addUnmappedPhones(ph);
                }
        }
    }
    public void attachPhonesOld(String res){
        if (phone==null){
            return;
        }
        Pattern p = Pattern.compile("((?<=office|mobile|phone|ph|tel|telephone|cellphone|" +
                "Office Telephone\\/Fax|\"Tel\\/Fax|Telephone number\\/fax|Telephone no|Tel & fax|Tel and fax|Mobile No|Telephone\\s\\/\\sFax|Tel No|Tel.)" +
                "(#|\\.|:\\.|\\.:|\\s:|:|\\s*)(\\s\\+?[(\\d\\s\\(\\)-)|(\\/|.)-]+))|" +
                "(\\+?[\\d\\/\\-\\s\\(\\)]*(\\(\\d{2,4}\\))*[\\d \\/\\-]+)(?=\\(off|\\(ph|\\(tel|\\(mob)|" +
                "(?)(^\\+\\d{2}-\\d{2}-\\d{3}-\\d{4})(?)",Pattern.CASE_INSENSITIVE);

        Matcher m = p.matcher(phone);
        String ph = null;

//        this pattern is like Tel.: +31 (0)20 – 566 1645
        Pattern p1 = Pattern.compile("\\+\\d+\\s*\\(\\d+\\)\\d+\\s+([–-])\\s+\\d+\\s+\\d+", Pattern.CASE_INSENSITIVE);

        Matcher m1 = p1.matcher(phone);
        while (m.find()){
            //phone appearing before the label
            if (m.group(3)!=null){
                ph = m.group(3).trim();
                if(m1.find() && ph.length() < m1.group(0).trim().length()) {
                    ph = m1.group(0).trim();
                }
            }
            //phone number appearing after the label
     ...
     */
      public void attachAffiliations() {
          ...
          AuthorAffiliation authAff = new AuthorAffiliation();
          ...
          if (hasMarker) {
              String specialCharacterGroup = "[+†#*]+";
              for (Person author : fullAuthors) {
                  List<String> authorMarkers = author.getMarkers();
                  if (authorMarkers != null) {
                      for (String authorMarker : authorMarkers) {
                          if (Pattern.matches("(" + specialCharacterGroup + "([A-z]+|[0-9]+))|(([A-z]+|[0-9]+)" + specialCharacterGroup + ")", authorMarker))
                              authorMarker = authorMarker.replaceAll(specialCharacterGroup, "");
                          for (Affiliation aff : fullAffiliations) {
                              if (aff.getMarker() != null) {
                                  if (aff.getMarker().equals(authorMarker)) {
          ...
          } else if (!authAff.doesAnyAffiliationHasMarker(sequenceTags)) {
              authAff.associateAuthorsWithAffiliations(sequenceTags, fullAffiliations, fullAuthors);
          }
      }
            ...
            if (nbAuthors > 0) {
                ...
                for (Person author : auts) {
                    ...
                    if (author.getCorresp()){
                        tei.append(" role=\"corresp\" sequence=\"" +
                                TextUtilities.HTMLEncode(author.getSeq().toString()) +"\">\n");
                    } else
                        tei.append(">\n");
                    ...
                    if (author.getMiddleName() != null) {
                        String[] middleNames = author.getMiddleName().split(" ");
                        for (String middleName : middleNames){
                            TextUtilities.appendN(tei, '\t', nbTag + 2);
                            tei.append("<forename type=\"middle\">" +
                                    TextUtilities.HTMLEncode(middleName) + "</forename>\n");
                    ...
                    if (author.getSuffix() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 2);
                        tei.append("<suffix>" +
                                TextUtilities.HTMLEncode(author.getSuffix()) + "</suffix>\n");
                    }
                    if (author.getPrefix() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 2);
                        tei.append("<prefix>" +
                                TextUtilities.HTMLEncode(author.getPrefix()) + "</prefix>\n");
                    }
                    if (author.getParticle() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 2);
                        tei.append("<particle>" +
                                TextUtilities.HTMLEncode(author.getParticle()) + "</particle>\n");
                    }
                    if (author.getAuthorDegree() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 2);
                        tei.append("<authordegree>" +
                                TextUtilities.HTMLEncode(author.getAuthorDegree()) + "</authordegree>\n");
                    }
                    ...
                    if (author.getEmail() != null || author.getPhones() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 1);
                        tei.append("<contact>\n");
                        if (author.getEmail() != null) {
                            TextUtilities.appendN(tei, '\t', nbTag + 2);
                            for (String e:author.getEmail()) {
                                tei.append("<email>" + TextUtilities.HTMLEncode(e) + "</email>\n");
                            }
//                            tei.append("<email>" + TextUtilities.HTMLEncode(author.getEmail()) + "</email>\n");
                        }

                        if (author.getPhones() != null) {
                            for(String ph : author.getPhones()) {
                                TextUtilities.appendN(tei, '\t', nbTag + 2);
                                tei.append("<phone>" + TextUtilities.HTMLEncode(ph.trim()) + "</phone>\n");
                            }
                        }

                        if (author.getUrls() != null) {
                            for(String wb : author.getUrls()) {
                                TextUtilities.appendN(tei, '\t', nbTag + 2);
                                tei.append("<url>" + TextUtilities.HTMLEncode(wb.trim()) + "</url>\n");
                            }
                        }
                    ...
                    if (author.getAffiliations() != null) {
                        ...
                        for (Affiliation aff : author.getAffiliations()) {
                            ...
                            /*
                            amn-mod on extra check on @ for discarding email content
                             */
                            if ((aff.getAddressString() != null && !aff.getAddressString().contains("@")) ||
                                    (aff.getAddrLine() != null && !aff.getAddrLine().contains("@")) ||
                                    (aff.getPostBox() != null) ||
                                    (aff.getPostCode() != null) ||
                                    (aff.getSettlement() != null) ||
                                    (aff.getRegion() != null && !aff.getRegion().contains("@")) ||
                                    (aff.getCountry() != null && !aff.getCountry().contains("@"))) {
                                TextUtilities.appendN(tei, '\t', nbTag + 2);

                                tei.append("<address>\n");
                                if (aff.getAddressString() != null && !aff.getAddressString().contains("@")) {
                                    TextUtilities.appendN(tei, '\t', nbTag + 3);
                                    String mod = FindSuperScript.FindSSAndModify(aff.getAddressString()) ;
                                    tei.append("<addrLine>" + TextUtilities.HTMLEncode(mod) +
                                            "</addrLine>\n");
                                }
                                if (aff.getAddrLine() != null && !aff.getAddrLine().contains("@")) {
                                    TextUtilities.appendN(tei, '\t', nbTag + 3);
                                    String mod = FindSuperScript.FindSSAndModify(aff.getAddrLine()) ;
                                    tei.append("<addrLine>" + TextUtilities.HTMLEncode(mod) +
                                            "</addrLine>\n");
                                }
                                if (aff.getPostBox() != null) {
                                    TextUtilities.appendN(tei, '\t', nbTag + 3);
                                    String mod = FindSuperScript.FindSSAndModify(aff.getPostBox()) ;
                                    tei.append("<postBox>" + TextUtilities.HTMLEncode(mod) +
                                            "</postBox>\n");
                                }
                                if (aff.getPostCode() != null) {
                                    TextUtilities.appendN(tei, '\t', nbTag + 3);
                                    String mod = FindSuperScript.FindSSAndModify(aff.getPostCode()) ;
                                    tei.append("<postCode>" + TextUtilities.HTMLEncode(mod) +
                                            "</postCode>\n");
                                }
                                ...
                                if (aff.getRegion() != null && !aff.getRegion().contains("@")) {
                                    TextUtilities.appendN(tei, '\t', nbTag + 3);
                                    tei.append("<region>" + TextUtilities.HTMLEncode(aff.getRegion()) +
                                            "</region>\n");
                                }
                                if (aff.getCountry() != null && !aff.getCountry().contains("@")) {
                    ...
                    if (author.getBiography() != null) {
                        TextUtilities.appendN(tei, '\t', nbTag + 2);
                        tei.append("<authorbiography>" +
                                TextUtilities.HTMLEncode(author.getBiography()) + "</authorbiography>\n");
                    }
                    ...
                    if (address != null) {
                        ...
                        while (st3.hasMoreTokens()) {
                            ...
                            if (addressRank == affiliationRank) {
                                TextUtilities.appendN(tei, '\t', nbTag + 2);
                                String mod = FindSuperScript.FindSSAndModify(add) ;
                                tei.append("<address><addrLine>" + TextUtilities.HTMLEncode(mod)
                                        + "</addrLine></address>\n");
                                break;
                            }
                            addressRank++;
    ...
    public String toTEIAuthorBlockNew(int nbTag) {
        StringBuffer tei = new StringBuffer();
        int nbAuthors = 0;
        int nbAffiliations = 0;
        int nbAddresses = 0;
                ...
                if(sequenceTags.size() > 0) {
                    for (ITag tag : sequenceTags) {
                        if(tag.getITagName().equals("I-<author>")) {
                            for (Person author: tag.getPersons()) {
...
//                            tei.append("<email>" + TextUtilities.HTMLEncode(author.getEmail()) + "</email>\n");
    ...
    public static void correct(BiblioItem bib, BiblioItem bibo) {
        ...
        if (bibo.getArticleNote() != null) {
            bib.setArticleNote(bibo.getArticleNote());
        }
    ...
    public void setCorresAttrToAuthor(List<Person> fullAuthors, String email) {
        if(fullAuthors != null && email.length() > 0) {
            for(Person per : fullAuthors) {
                if(per.getEmail() != null) {
                    for(String e: per.getEmail()) {
                        if(email.contains(e)) {
                            per.setCorresp(true);
                            break;
                        }
    ...
    public void checkAuthorMarkerHasAstrik() {
        for (Person p : this.fullAuthors) {
            if (p.getMarkers() != null && p.doesHaveSymbolicMarker()) {
                p.setAstrikPresence(true);
                this.doesAnyAuthorHasAstrik = true;
            }
        }
    }

    public void setCorrespondingAuthor(){
	    if(this.doesAnyAuthorHasAstrik) {
            int seq = 1;
            for(Person auth:this.fullAuthors){
                if(auth.getAstrikPresence() && doesAuthorHasEmail(auth)){
                    seq = setAuthorAsCorresponding(seq, auth);
                } else {
                    auth.setCorresp(false);
                }
            }
        } else if(this.doesHaveContextualWord()) {
            int seq = 1;
            for(Person auth:this.fullAuthors){
                if(auth.getCorresp() && doesAuthorHasEmail(auth)){
                    seq = setAuthorAsCorresponding(seq, auth);
                } else {
                    auth.setCorresp(false);
                }
    ...
    private int setAuthorAsCorresponding(int seq, Person auth) {
        auth.setCorrespondingSeq(seq);
        auth.setCorresp(true);
        seq += 1;
        return seq;
    }

    private boolean doesAuthorHasEmail(Person auth) {
        return auth.getEmail() != null && auth.getEmail().size() > 0;
    }

    public boolean doesHaveContextualWord() {
        return doesHasContextualWord;
    }

    public void setContextualWord(boolean doesHasContextualWord) {
        this.doesHasContextualWord = doesHasContextualWord;
    }

    public boolean doesAnyAuthorHasAstrik(){
	    return this.doesAnyAuthorHasAstrik;
    }

    public void setdoesAnyAuthorHasAstrik(boolean value){
        this.doesAnyAuthorHasAstrik = value;
    }
