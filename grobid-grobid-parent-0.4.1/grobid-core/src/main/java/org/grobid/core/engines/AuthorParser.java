...
//            tagger.delete();
            ...
            while (st2.hasMoreTokens()) {
                ...
                if (s2.contains("corresponding")){
                    if (s4 == "LINESTART" | s5 == "ALLCAPS"){
                        corresTextFlag = true;
                    }
                }
...
//                        } else if (aut.getSuffix() != null) {
                ...
                } else if (s1.equals("<particle>") || s1.equals("I-<particle>")) {
                    if (s3.equals("<author>")) {
//                        if (newMarker) {
//                            aut.setParticle(s2);
//                            newMarker = false;
//                        } else if (aut.getParticle() != null) {
                        if (aut.getParticle() != null) {
                            if (s1.equals("I-<particle>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut = new Person();
                                aut.setParticle(s2);
                            } else if (!s1.equals(lastTag) && !lastTag.equals("I-<particle>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut.setParticle(aut.getParticle() + s2);
                            else {
                                if (aut.getParticle().length() == 0)
                                    aut.setParticle(aut.getParticle() + s2);
                                else if ((aut.getParticle().charAt(aut.getParticle().length() - 1) == '-')
                                        | (aut.getParticle().charAt(aut.getParticle().length() - 1) == '\''))
                                    aut.setParticle(aut.getParticle() + s2);
                                else
                                    aut.setParticle(aut.getParticle() + " " + s2);
                            }
                ...
                } else if (s1.equals("<authordegree>") || s1.equals("I-<authordegree>")) {
                    if (s3.equals("<author>")) {
//                        if (newMarker) {
//                            aut.setAuthorDegree(s2);
//                            newMarker = false;
//                        } else if (aut.getAuthorDegree() != null) {
                        if (aut.getAuthorDegree() != null) {
                            if (s1.equals("I-<authordegree>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut = new Person();
                                aut.setAuthorDegree(s2);
                            } else if (!s1.equals(lastTag) && !lastTag.equals("I-<authordegree>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut.setAuthorDegree(aut.getAuthorDegree() + s2);
                            else {
                                if (aut.getAuthorDegree().length() == 0)
                                    aut.setAuthorDegree(aut.getAuthorDegree() + s2);
                                else if ((aut.getAuthorDegree().charAt(aut.getAuthorDegree().length() - 1) == '-')
                                        | (aut.getAuthorDegree().charAt(aut.getAuthorDegree().length() - 1) == '\''))
                                    aut.setAuthorDegree(aut.getAuthorDegree() + s2);
                                else
                                    aut.setAuthorDegree(aut.getAuthorDegree() + " " + s2);
                            }
                ...
                } else if (s1.equals("<authorbiography>") || s1.equals("I-<authorbiography>")) {
                    if (s3.equals("<author>")) {
//                        if (newMarker) {
//                            aut.setBiography(s2);
//                            newMarker = false;
//                        } else if (aut.getBiography() != null) {
                        if (aut.getBiography() != null) {
                            if (s1.equals("I-<authorbiography>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut = new Person();
                                aut.setBiography(s2);
                            } else if (!s1.equals(lastTag) && !lastTag.equals("I-<authorbiography>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut.setBiography(aut.getBiography() + s2);
                            else {
                                if (aut.getBiography().length() == 0)
                                    aut.setBiography(aut.getBiography() + s2);
                                else if ((aut.getBiography().charAt(aut.getBiography().length() - 1) == '-')
                                        | (aut.getBiography().charAt(aut.getBiography().length() - 1) == '\''))
                                    aut.setBiography(aut.getBiography() + s2);
                                else
                                    aut.setBiography(aut.getBiography() + " " + s2);
                            }
                ...
                } else if (s1.equals("<prefix>") || s1.equals("I-<prefix>")) {
                    if (s3.equals("<author>")) {
                        if (aut.getPrefix() != null) {
                            if (s1.equals("I-<prefix>")) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut = new Person();
                                aut.setPrefix(s2);
                            } else if ((!s1.equals(lastTag)) && (!lastTag.equals("I-<prefix>"))) {
                                // new author
                                if (aut.notNull()) {
                                    if (fullAuthors == null)
                                        fullAuthors = new ArrayList<Person>();
                                ...
                                aut.setPrefix(aut.getPrefix() + s2);
                            } else {
                                if (aut.getPrefix().length() == 0) {
                                    aut.setPrefix(aut.getPrefix() + s2);
                                } else if (
                                        (aut.getPrefix().charAt(aut.getPrefix().length() - 1) == '-') |
                                                (aut.getPrefix().charAt(aut.getPrefix().length() - 1) == '\'')) {
                                    aut.setPrefix(aut.getPrefix() + s2);
                                } else {
                                    aut.setPrefix(aut.getPrefix() + " " + s2);
...
//            Tagger tagger = head ? taggerHeader : taggerCitation;
            ...
            while (st2.hasMoreTokens()) {
                ...
                if (output == null) {
                    output = writeField(s1, lastTag0, s2, "<authordegree>", "<authordegree>", addSpace, 8, head);
                } else {
                    buffer.append(output);
                    lastTag = s1;
                    continue;
                ...
                if (output == null) {
                    output = writeField(s1, lastTag0, s2, "<particle>", "<particle>", addSpace, 8, head);
                } else {
                    buffer.append(output);
                    lastTag = s1;
                    continue;
                ...
                if (output == null) {
                    output = writeField(s1, lastTag0, s2, "<prefix>", "<prefix>", addSpace, 8, head);
                } else {
                    buffer.append(output);
                    lastTag = s1;
                    continue;
    ...
    private boolean testClosingTag(StringBuilder buffer,
        ...
        if (!currentTag0.equals(lastTag0)) {
            ...
            if (lastTag0.equals("<other>")) {
				...
				if (head)
					buffer.append("\n");
            } else if (lastTag0.equals("<authordegree>")) {
                buffer.append("</authordegree>");
                if (head)
                    buffer.append("\n");
            } else if (lastTag0.equals("<prefix>")) {
                buffer.append("</prefix>");
                if (head)
                    buffer.append("\n");
            } else if (lastTag0.equals("<particle>")) {
